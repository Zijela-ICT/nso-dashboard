    ...
    useEffect(() => {
        ...
        const confirmNavigation = (callback) => {
            ...
            if (confirmLeave) {
                setIsEditting(false);
                callback();
                unlockBook({id: String(currentBook.id)})
            }
        };
                    ...
                    {hasEditAccess && isEditting && (
                        <>
                            <UploadFileModal onChange={handleFileUpload}>
                                <button
                                    className="fixed bottom-[120px] right-12 bg-[#136c2a] text-white px-4 h-[60px] w-[60px] rounded-full flex items-center justify-center shadow-md hover:shadow-lg">
                                    <Upload/>
                                </button>
                            </UploadFileModal>
                            <button
